    ...
    if (coordinates || positions) {
      ...
      if (coordinates.length) {
        ...
        map.fitBounds(bounds, {
          padding: {
            left: Math.min(canvas.width, canvas.height) * 0.1 + 350,
            right: Math.min(canvas.width, canvas.height) * 0.1,
            top: Math.min(canvas.width, canvas.height) * 0.1,
            bottom: Math.min(canvas.width, canvas.height) * 0.1
          },
          duration: 0,
        });
      }
